# Onion Saronite Blockchain Explorer

Currently available Saronite blockchain explorers have several limitations which are of 
special importance to privacy-oriented users:
 ...
 - do not support Saronite testnet nor stagenet networks,
 - have limited JSON API.


In this example, these limitations are addressed by development of
an Onion Saronite Blockchain Explorer. The example not only shows how to use 
Saronite C++ libraries, but also demonstrates how to use:
...
## Onion Saronite Blockchain Explorer features
 ...
 - showing public components of Saronite addresses,
 - decoding which outputs and mixins belong to the given Saronite address and viewkey,
 - can prove that you send Saronite to someone,
 - detailed information about mixins, such as, mixins' age, timescale, mixin of mixins,
 - showing number of amount output indices,
 - support Saronite testnet, stagenet network,
...
Current development branch:
 ...
 - https://github.com/saronite/onion-saronite-blockchain-explorer



## Compilation on Ubuntu 16.04/18.04

##### Compile latest Saronite development version

Download and compile recent Saronite into your home folder:

```bash
# first install Saronite dependecines
sudo apt update
...
git clone --recursive https://github.com/saronite/saronite
...
Once the Saronite is compiles, the explorer can be downloaded and compiled
as follows:

```bash
# go to home folder if still in ~/saronite
cd ~

# download the source code
git clone https://github.com/saronite/onion-saronite-blockchain-explorer.git

# enter the downloaded sourced code folder
cd onion-saronite-blockchain-explorer
...
# alternatively can use: cmake -DSARONITE_DIR=/path/to/saronite ..
# if saronite is not in ~/saronite
#
...
When compilation finishes executable `xrnblocks` should be created. Before running
...
By default it will look for blockchain in its default location i.e., `~/.bitsaronite/lmdb`.
...
```
xrnblocks, Onion Saronite Blockchain Explorer:
  ...
  --enable-emission-monitor [=arg(=1)] (=0)
                                        enable Saronite total emission monitoring
                                        thread
...
alias xrnblocksmainnet='~/onion-saronite-blockchain-explorer/build/xrnblocks    --port 8081 --testnet-url "http://139.162.32.245:8082" --enable-pusher --enable-emission-monitor'

# for testnet explorer
alias xrnblockstestnet='~/onion-saronite-blockchain-explorer/build/xrnblocks -t --port 8082 --mainnet-url "http://139.162.32.245:8081" --enable-pusher --enable-emission-monitor'
```

These are aliases similar to those used for http://139.162.32.245:8081/ and http://139.162.32.245:8082/, respectively.

## Enable Saronite emission 

Obtaining current Saronite emission amount is not straight forward. Thus, by default it is 
disabled. To enable it use `--enable-emission-monitor` flag, e.g., 


```bash
xrnblocks --enable-emission-monitor 
```
...
Every 10000 blocks, the thread will save current emission in a file, by default, 
 in `~/.bitsaronite/lmdb/emission_amount.txt`. For testnet or stagenet networks, 
 it is `~/.bitsaronite/testnet/lmdb/emission_amount.txt` or `~/.bitsaronite/stagenet/lmdb/emission_amount.txt`. This file is used so that we don't
 need to rescan entire blockchain whenever the explorer is restarted. When the 
 explorer restarts, the thread will first check if `~/.bitsaronite/lmdb/emission_amount.txt`
...
```
Saronite emission (fees) is 14485540.430 (52545.373) as of 1313448 block
```

The values given, can be checked using Saronite daemon's  `print_coinbase_tx_sum` command. 
...
Having the `crt` and `key` files, run `xrnblocks` in the following way:

```bash
./xrnblocks --ssl-crt-file=/tmp/server.crt --ssl-key-file=/tmp/server.key 
```
...
We use recipient's address (i.e. not our address from which we sent xrn to recipient).
...
```json
{
  "data": {
    ...
    "git_branch_name": "update_to_current_saronite",
    "last_git_commit_date": "2017-07-25",
    "last_git_commit_hash": "a549f25",
    "saronite_version_full": "0.10.3.1-ab594cfe"
  },
  "status": "success"
}
```
...
Return raw json block data, as represented in Saronite.
...
Return raw json tx data, as represented in Saronite.
