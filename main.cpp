...
main(int ac, const char* av[])
{
    ...
    ([&](const crow::request& req) {
        return crow::response(xrnblocks.index2());
    });

    CROW_ROUTE(app, "/page/<uint>")
    ([&](size_t page_no) {
        return xrnblocks.index2(page_no);
    });

    CROW_ROUTE(app, "/block/<uint>")
    ([&](const crow::request& req, size_t block_height) {
        return crow::response(xrnblocks.show_block(block_height));
    });

    CROW_ROUTE(app, "/block/<string>")
    ([&](const crow::request& req, string block_hash) {
        return crow::response(xrnblocks.show_block(remove_bad_chars(block_hash)));
    });

    CROW_ROUTE(app, "/tx/<string>")
    ([&](const crow::request& req, string tx_hash) {
        return crow::response(xrnblocks.show_tx(remove_bad_chars(tx_hash)));
    });

    CROW_ROUTE(app, "/tx/<string>/<uint>")
    ([&](string tx_hash, uint16_t with_ring_signatures)
     {
        return xrnblocks.show_tx(remove_bad_chars(tx_hash), with_ring_signatures);
    });

    CROW_ROUTE(app, "/service_node/<string>")
    ([&](const crow::request& req, std::string service_node_pubkey) {
        return crow::response(xrnblocks.show_service_node(remove_bad_chars(service_node_pubkey)));
    });

    CROW_ROUTE(app, "/service_nodes")
    ([&](const crow::request& req) {
        return xrnblocks.render_service_nodes_html(true /*add_header_and_footer*/);
    });

    CROW_ROUTE(app, "/quorums")
    ([&](const crow::request& req) {
        return xrnblocks.render_quorum_states_html(true /*add_header_and_footer*/);
    });

    // TODO(saronite): This should be combined into the normal search mechanism, we shouldn't have 2 search bars.
    CROW_ROUTE(app, "/search_service_node").methods("GET"_method)
    ([&](const crow::request& req) {
        return xrnblocks.show_service_node(remove_bad_chars(string(req.url_params.get("value"))));
    });
    ...
    ([&](const crow::request& req)
     {
        ...
        if (post_body.count("xrn_address") == 0
            || post_body.count("viewkey") == 0
            || post_body.count("tx_hash") == 0)
        {
            return string("xrn address, viewkey or tx hash not provided");
        }

        string tx_hash     = remove_bad_chars(post_body["tx_hash"]);
        string xrn_address = remove_bad_chars(post_body["xrn_address"]);
        ...
        return xrnblocks.show_my_outputs(tx_hash, xrn_address,
                                         viewkey, raw_tx_data,
                                         domain);
    });
    ...
    ([&](const crow::request& req, string tx_hash,
        string xrn_address, string viewkey)
     {

        string domain = get_domain(req);

        return xrnblocks.show_my_outputs(remove_bad_chars(tx_hash),
                                         remove_bad_chars(xrn_address),
    ...
    CROW_ROUTE(app, "/prove").methods("POST"_method)
        ([&](const crow::request& req) {
            ...
            if (post_body.count("xrnaddress") == 0
                || post_body.count("txprvkey") == 0
                || post_body.count("txhash") == 0)
            {
                return string("xrn address, tx private key or "
            ...
            string xrn_address = remove_bad_chars(post_body["xrnaddress"]);
            ...
            return xrnblocks.show_prove(tx_hash,
                                        xrn_address,
                                        tx_prv_key,
                                        raw_tx_data,
                                        domain);
    ...
    ([&](const crow::request& req, string tx_hash,
         string xrn_address, string tx_prv_key) {

        string domain = get_domain(req);

        return xrnblocks.show_prove(remove_bad_chars(tx_hash),
                                    remove_bad_chars(xrn_address),
    ...
    if (enable_pusher)
    {
        ...
        ([&](const crow::request& req) {
            ...
            if (action == "check")
                return xrnblocks.show_checkrawtx(raw_tx_data, action);
            else if (action == "push")
                return xrnblocks.show_pushrawtx(raw_tx_data, action);
    ...
    if (enable_key_image_checker)
    {
        ...
        ([&](const crow::request& req) {
            return xrnblocks.show_rawkeyimgs();
        });
        ...
        ([&](const crow::request& req) {
            ...
            return xrnblocks.show_checkrawkeyimgs(raw_data, viewkey);
        });
    }
    ...
    if (enable_output_key_checker)
    {
        ...
        ([&](const crow::request& req) {
            return xrnblocks.show_rawoutputkeys();
        });
        ...
        ([&](const crow::request& req) {
            ...
            return xrnblocks.show_checkcheckrawoutput(raw_data, viewkey);
        });
    }
    ...
    ([&](const crow::request& req) {
        return xrnblocks.search(remove_bad_chars(string(req.url_params.get("value"))));
    });

    CROW_ROUTE(app, "/mempool")
    ([&](const crow::request& req) {
        return xrnblocks.mempool(true);
    });
...
//    });
    ...
    if (enable_js)
    {
        ...
        ([&](const crow::request& req) {
            return xrnblocks.get_js_file("jquery.min.js");
        });

        CROW_ROUTE(app, "/js/crc32.js")
        ([&](const crow::request& req) {
            return xrnblocks.get_js_file("crc32.js");
        });

        CROW_ROUTE(app, "/js/biginteger.js")
        ([&](const crow::request& req) {
            return xrnblocks.get_js_file("biginteger.js");
        });

        CROW_ROUTE(app, "/js/crypto.js")
        ([&](const crow::request& req) {
            return xrnblocks.get_js_file("crypto.js");
        });

        CROW_ROUTE(app, "/js/config.js")
        ([&](const crow::request& req) {
            return xrnblocks.get_js_file("config.js");
        });

        CROW_ROUTE(app, "/js/nacl-fast-cn.js")
        ([&](const crow::request& req) {
            return xrnblocks.get_js_file("nacl-fast-cn.js");
        });

        CROW_ROUTE(app, "/js/base58.js")
        ([&](const crow::request& req) {
            return xrnblocks.get_js_file("base58.js");
        });

        CROW_ROUTE(app, "/js/cn_util.js")
        ([&](const crow::request& req) {
            return xrnblocks.get_js_file("cn_util.js");
        });

        CROW_ROUTE(app, "/js/sha3.js")
        ([&](const crow::request& req) {
            return xrnblocks.get_js_file("sha3.js");
        });
        ...
        ([&](const crow::request& req) {
            ...
            return xrnblocks.get_js_file("all_in_one.js");
        });
    ...
    if (enable_json_api)
    {
        ...
        ([&](const crow::request &req, string tx_hash) {
            ...
            myxrn::jsonresponse r{xrnblocks.json_transaction(remove_bad_chars(tx_hash))};
        ...
        ([&](const crow::request &req, string tx_hash) {
            ...
            myxrn::jsonresponse r{xrnblocks.json_rawtransaction(remove_bad_chars(tx_hash))};
        ...
        ([&](const crow::request &req, string block_no_or_hash) {
            ...
            myxrn::jsonresponse r{xrnblocks.json_block(remove_bad_chars(block_no_or_hash))};
        ...
        ([&](const crow::request &req, string block_no_or_hash) {
            ...
            myxrn::jsonresponse r{xrnblocks.json_rawblock(remove_bad_chars(block_no_or_hash))};
        ...
        ([&](const crow::request &req) {
            ...
            myxrn::jsonresponse r{xrnblocks.json_transactions(
        ...
        ([&](const crow::request &req) {
            ...
            myxrn::jsonresponse r{xrnblocks.json_mempool(
                    remove_bad_chars(page), remove_bad_chars(limit))};
        ...
        ([&](const crow::request &req, string search_value) {
            ...
            myxrn::jsonresponse r{xrnblocks.json_search(remove_bad_chars(search_value))};
        ...
        ([&](const crow::request &req) {
            ...
            myxrn::jsonresponse r{xrnblocks.json_networkinfo()};
        ...
        ([&](const crow::request &req) {
            ...
            myxrn::jsonresponse r{xrnblocks.json_emission()};
        ...
        ([&](const crow::request &req) {
            ...
            myxrn::jsonresponse r{xrnblocks.json_outputs(
        ...
        ([&](const crow::request &req) {
            ...
            myxrn::jsonresponse r{xrnblocks.json_outputsblocks(
        ...
        ([&](const crow::request &req) {
            ...
            myxrn::jsonresponse r{xrnblocks.json_version()};
    ...
    if (enable_autorefresh_option)
    {
        ...
        ([&]() {
            ...
            return xrnblocks.index2(page_no, refresh_page);
        });
    }
