...
MempoolStatus::read_mempool()
{
    ...
    for (size_t i = 0; i < mempool_tx_info.size(); ++i)
    {
        ...
        double payed_for_kB = XRN_AMOUNT(_tx_info.fee) / tx_size;
        ...
        last_tx.fee_str          = lokeg::xrn_amount_to_str(_tx_info.fee, "{:0.3f}", false);
        last_tx.payed_for_kB_str = fmt::format("{:0.4f}", payed_for_kB);
        last_tx.xrn_inputs_str   = lokeg::xrn_amount_to_str(last_tx.sum_inputs , "{:0.3f}");
        last_tx.xrn_outputs_str  = lokeg::xrn_amount_to_str(last_tx.sum_outputs, "{:0.3f}");
...
bf::path MempoolStatus::blockchain_path {"/home/mwo/.bitsaronite/lmdb"};
string MempoolStatus::daemon_url {"http:://127.0.0.1:31014"};
