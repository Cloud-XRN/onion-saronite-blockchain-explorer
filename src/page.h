...
#ifndef CROWXRN_PAGE_H
#define CROWXRN_PAGE_H



#include "mstch/mstch.hpp"

#include "saronite_headers.h"
...
struct tx_details
{
    ...
    // public keys and xrn amount of outputs
    ...
    get_mstch_map() const
    {
        ...
        const double& xrn_amount = XRN_AMOUNT(fee);
        ...
        {
            double payed_for_kB = xrn_amount / tx_size;

            mixin_str        = std::to_string(mixin_no);
            fee_str          = fmt::format("{:0.6f}", xrn_amount);
            fee_short_str    = fmt::format("{:0.3f}", xrn_amount);
        ...
        mstch::map txd_map {
                ...
                {"sum_inputs"        , xrn_amount_to_str(xrn_inputs , "{:0.6f}")},
                {"sum_outputs"       , xrn_amount_to_str(xrn_outputs, "{:0.6f}")},
                {"sum_inputs_short"  , xrn_amount_to_str(xrn_inputs , "{:0.3f}")},
                {"sum_outputs_short" , xrn_amount_to_str(xrn_outputs, "{:0.3f}")},
...
public:
    ...
    index2(uint64_t page_no = 0, bool refresh_page = false)
    {
        ...
        std::future<json> network_info_ftr = std::async(std::launch::async, [&]
        {
            ...
            get_saronite_network_info(j_info);
        ...
        if (CurrentBlockchainStatus::is_thread_running())
        {
            ...
            string emission_coinbase = xrn_amount_to_str(current_values.coinbase, "{:0.3f}");
            string emission_fee      = xrn_amount_to_str(current_values.fee, "{:0.3f}");
    ...
    mempool(bool add_header_and_footer = false, uint64_t no_of_mempool_tx = 25)
    {
        ...
        for (size_t i = 0; i < no_of_mempool_tx; ++i)
        {
            ...
            txs.push_back(mstch::map {
                    ...
                    {"xrn_inputs"      , mempool_tx.xrn_inputs_str},
                    {"xrn_outputs"     , mempool_tx.xrn_outputs_str},
    ...
    show_block(uint64_t _blk_height)
    {
        ...
        context["sum_fees"]
                = lokeg::xrn_amount_to_str(sum_fees, "{:0.6f}", false);

        // get xrn in the block reward
        context["blk_reward"]
                = lokeg::xrn_amount_to_str(txd_coinbase.xrn_outputs - sum_fees, "{:0.6f}");
    ...
    show_service_node(const std::string &service_node_pubkey)
    {
        ...
        else
        {
          ...
          snprintf(buf, sizeof(buf),
              "This service node is awaiting to be registered and has: %s saronite to be contributed remaining",
                    ...
                    bool tx_prove = false)
    {
        ...
        if (xrn_address_str.empty())
        {
            return string("Saronite address not provided!");
        }
        ...
        // parse string representing given saronite address
        cryptonote::address_parse_info address_info;

        if (!lokeg::parse_str_address(xrn_address_str,  address_info, nettype))
        {
            cerr << "Cant parse string address: " << xrn_address_str << endl;
            return string("Cant parse xrn address: " + xrn_address_str);
        }
...
//        }
        ...
        mstch::map context {
                ...
                {"xrn_address"          , xrn_address_str},
                {"viewkey"              , viewkey_str_partial},
                {"tx_pub_key"           , pod_to_hex(txd.pk)},
                {"blk_height"           , tx_blk_height_str},
                {"tx_size"              , fmt::format("{:0.4f}",
                                                      static_cast<double>(txd.size) / 1024.0)},
                {"tx_fee"               , lokeg::xrn_amount_to_str(txd.fee, "{:0.12f}", true)},
                {"blk_timestamp"        , blk_timestamp},
        ...
        for (pair<txout_to_key, uint64_t>& outp: txd.output_pub_keys)
        {
            ...
            // if someone had sent us some xrn.
            ...
            outputs.push_back(mstch::map {
                    ...
                    {"amount"                , lokeg::xrn_amount_to_str(outp.second)},
        ...
        // to hold sum of xrn in matched mixins, those that
        // perfectly match mixin public key with outputs in mixn_tx.
        uint64_t sum_mixin_xrn {0};
        ...
        for (const txin_to_key& in_key: input_key_imgs)
        {
            ...
            inputs.push_back(mstch::map{
                    ...
                    {"key_image_amount", lokeg::xrn_amount_to_str(in_key.amount)},
...
//                        }
                    ...
                    found_outputs.push_back(mstch::map {
                            ...
                            {"amount"          , lokeg::xrn_amount_to_str(amount)}
                    });
                    ...
                    if (mine_output)
                    {
                        ...
                        // increase sum_mixin_xrn only when
...
//                                << endl;
        ...
        context["found_our_outputs"] = (sum_xrn > 0);
        context["sum_xrn"]           = lokeg::xrn_amount_to_str(sum_xrn);

        context.emplace("inputs", inputs);

        context["show_inputs"]   = show_key_images;
        context["inputs_no"]     = static_cast<uint64_t>(inputs.size());
        context["sum_mixin_xrn"] = lokeg::xrn_amount_to_str(
                sum_mixin_xrn, "{:0.12f}", false);
        ...
        if (sum_mixin_xrn > (sum_xrn + txd.fee)
            && no_of_matched_mixins == inputs.size())
        {
            //                  (outcoming    - incoming) - fee
            possible_spending = (sum_mixin_xrn - sum_xrn) - txd.fee;
        }

        context["possible_spending"] = lokeg::xrn_amount_to_str(
               ...
               string domain)
    {
        ...
        return show_my_outputs(tx_hash_str, xrn_address_str,
    ...
    show_checkrawtx(string raw_tx_data, string action)
    {
            ...
            if (r)
            {
                ...
                for (const ::tools::wallet2::tx_construction_data& tx_cd: exported_txs.txes)
                {
                    ...
                    mstch::map tx_cd_data {
                            ...
                            {"change_amount"      , lokeg::xrn_amount_to_str(tx_change.amount)},
                            {"has_payment_id"     , (payment_id  != null_hash)},
                            {"has_payment_id8"    , (payment_id8 != null_hash8)},
                            {"payment_id"         , pid_str},
                            {"payment_id8"        , pid8_str},
                    ...
                    for (const tx_destination_entry& a_dest: tx_cd.splitted_dsts)
                    {
                        mstch::map dest_info {
                                ...
                                {"dest_amount"   , lokeg::xrn_amount_to_str(a_dest.amount)}
                        };
                    ...
                    for (size_t i = 0; i < no_of_sources; ++i)
                    {
                        ...
                        mstch::map single_dest_source {
                                {"output_amount"              , lokeg::xrn_amount_to_str(tx_source.amount)},
                    ...
                    tx_cd_data.insert({"sum_outputs_amounts" ,
                                       lokeg::xrn_amount_to_str(sum_outputs_amounts)});
        ...
        else
        {
            ...
            if (strncmp(decoded_raw_tx_data.c_str(), SIGNED_TX_PREFIX, magiclen) != 0)
            {
                ...
                // used in mysaronite and opensaronite projects.
            ...
            for (tools::wallet2::pending_tx& ptx: ptxs)
            {
                ...
                if (ptx.construction_data.change_dts.amount > 0)
                {
                    destination_addresses.push_back(
                            mstch::map {
                                    ...
                                    {"dest_amount"    ,
                                            lokeg::xrn_amount_to_str(ptx.construction_data.change_dts.amount)},
                                    {"is_this_change" , true}
                            }
                    );
                ...
                tx_context["outputs_xrn_sum"] = lokeg::xrn_amount_to_str(outputs_xrn_sum);
                ...
                for (size_t i = 0; i < outputs.size(); ++i)
                {
                    ...
                    if (parse_amount(output_amount, out_amount_str))
                    {
                        if (output_amount == 0)
                        {
                            out_amount_str = lokeg::xrn_amount_to_str(real_ammounts.at(i));
                        }
                    }
                }
                ...
                for (const tx_source_entry&  tx_source: ptx.construction_data.sources)
                {
                    ...
                    inputs_xrn_sum += tx_source.amount;
                }

                // mark that we have signed tx data for use in mstch
                tx_context["have_raw_tx"] = true;

                // provide total mount of inputs xrn
                tx_context["inputs_xrn_sum"] = lokeg::xrn_amount_to_str(inputs_xrn_sum);
                ...
                for (mstch::node& input_node: inputs)
                {
                    ...
                    amount = lokeg::xrn_amount_to_str(real_amounts.at(input_idx));
    ...
    show_pushrawtx(string raw_tx_data, string action)
    {
        ...
        // if failed, treat raw_tx_data as base64 encoding of signed_saronite_tx
        else
        {
            string decoded_raw_tx_data = epee::string_encoding::base64_decode(raw_tx_data);
    ...
    show_checkrawkeyimgs(string raw_data, string viewkey_str)
    {
        ...
        // get xrn address stored in this key image file
        const account_public_address* xrn_address =
                reinterpret_cast<const account_public_address*>(
                        decoded_raw_data.data());

        address_parse_info address_info {*xrn_address, false};
        ...
        context.insert({"has_total_xrn"  , false});
        context.insert({"total_xrn"      , string{}});
    ...
    show_checkcheckrawoutput(string raw_data, string viewkey_str)
    {
        ...
        address_parse_info address_info {*xrn_address, false, false, crypto::null_hash8};
        ...
        for (const tools::wallet2::transfer_details& td: outputs)
        {
            ...
            mstch::map output_info {
                    ...
                    {"amount"              , lokeg::xrn_amount_to_str(xrn_amount)},
        ...
        if (total_xrn > 0)
        {
            context["has_total_xrn"] = true;
            context["total_xrn"] = lokeg::xrn_amount_to_str(total_xrn);
        }
    ...
    search(string search_text)
    {
        ...
        // check if saronite address is given based on its length
        ...
        // check if integrated saronite address is given based on its length
    ...
    show_address_details(const address_parse_info& address_info, cryptonote::network_type nettype = cryptonote::network_type::MAINNET)
    {
        ...
        mstch::map context {
                {"xrn_address"        , REMOVE_HASH_BRAKETS(address_str)},
    ...
    json_rawtransaction(string tx_hash_str)
    {
        ...
        // get raw tx json as in saronite
 ...
 */
                 ...
                 bool tx_prove = false)
    {
        ...
        {
            j_response["status"]  = "error";
            j_response["message"] = "Saronite address not provided";
            return j_response;
        }
        ...
        {
            j_response["status"]  = "error";
            j_response["message"] = "Cant parse Saronite address: " + address_str;
            return j_response;
    ...
    json_networkinfo()
    {
        ...
        if (!get_saronite_network_info(j_info))
        {
            j_response["status"]  = "error";
            j_response["message"] = "Cant get Saronite network info";
            return j_response;
        }
...
private:
    ...
    get_tx_json(const transaction& tx, const tx_details& txd)
    {
        ...
        json j_tx {
                ...
                {"mixin"       , txd.mixin_no},
                {"tx_size"     , txd.size},
                {"xrn_outputs" , txd.xrn_outputs},
                {"xrn_inputs"  , txd.xrn_inputs},
    ...
    construct_tx_context(transaction tx, uint16_t with_ring_signatures = 0)
    {
        ...
        double payed_for_kB = XRN_AMOUNT(txd.fee) / tx_size;
        ...
        mstch::map context {
                ...
                {"tx_size"               , fmt::format("{:0.4f}", tx_size)},
                {"tx_fee"                , lokeg::xrn_amount_to_str(txd.fee, "{:0.12f}", false)},
        ...
        for (const txin_to_key &in_key: txd.input_key_imgs)
        {
            ...
            inputs.push_back(mstch::map {
                    ...
                    {"amount"       , lokeg::xrn_amount_to_str(in_key.amount)},
        ...
        context["inputs_xrn_sum_not_zero"]  = (inputs_xrn_sum > 0);
        context["inputs_xrn_sum"]           = lokeg::xrn_amount_to_str(inputs_xrn_sum);
        ...
        context["outputs_xrn_sum"] = lokeg::xrn_amount_to_str(outputs_xrn_sum);
                   ...
                   uint64_t bc_height = 0)
    {
        ...
        // sum xrn in inputs and ouputs in the given tx
        const array<uint64_t, 4>& sum_data = summary_of_in_out_rct(
                tx, txd.output_pub_keys, txd.input_key_imgs);

        txd.xrn_outputs       = sum_data[0];
        txd.xrn_inputs        = sum_data[1];
    ...
    bool
    get_saronite_network_info(json& j_info)
    {
