...
#ifndef CROWXRN_RPCCALLS_H
#define CROWXRN_RPCCALLS_H
...
{
// declare struct in saronite's cryptonote namespace.
// saronite should provide definition for this,
...
public:
    ...
    get_alt_blocks(vector<string>& alt_blocks_hashes)
    {
        ...
        if (r)
        {
            ...
            if (!err.empty())
            {
                cerr << "Error connecting to Saronite daemon due to "
                     << err << endl;
                return false;
            }
        }
        ...
        {
            cerr << "Error connecting to Saronite daemon at "
                 << daemon_url << endl;
            return false;
        }
