...
/**
* Parse saronite address in a string form into
...
/**
* Return string representation of saronite address
*/
string
...
{
    // default path to saronite folder
    // on linux this is /home/<username>/.saronite
    string default_saronite_dir = tools::get_default_data_dir();

    if (nettype == cryptonote::network_type::TESTNET)
        default_saronite_dir += "/testnet";
    if (nettype == cryptonote::network_type::STAGENET)
        default_saronite_dir += "/stagenet";


    // the default folder of the lmdb blockchain database
    // is therefore as follows
    return default_saronite_dir + string("/lmdb");
}
...
{
    pair<uint64_t, uint64_t> sum_xrn {0, 0};
    ...
    {
        sum_xrn.first += vout["amount"].get<uint64_t>();
        ++sum_xrn.second;
    }
...
{
    pair<uint64_t, uint64_t> sum_xrn {0ULL, 0ULL};
        ...
        vector<txin_to_key>& input_key_imgs)
{
    ...
    uint64_t xrn_outputs       {0};
    uint64_t xrn_inputs        {0};
    uint64_t mixin_no          {0};
    ...
    for (size_t i = 0; i < input_no; ++i)
    {
        ...
        xrn_inputs += tx_in_to_key.amount;
    ...
    return {xrn_outputs, xrn_inputs, mixin_no, num_nonrct_inputs};
};
...
summary_of_in_out_rct(const json& _json)
{
    ...
    for (const json& vout: _json["vout"])
    {
        xrn_outputs += vout["amount"].get<uint64_t>();
    }
    ...
    return {xrn_outputs, xrn_inputs, no_outputs, no_inputs, mixin_no, num_nonrct_inputs};
};
...
sum_money_in_inputs(const string& json_str)
{
    ...
    for (json& vin: j["vin"])
    {
        sum_xrn.first += vin["key"]["amount"].get<uint64_t>();
        ++sum_xrn.second;
    }
...
{
    array<uint64_t, 2> sum_xrn;

    sum_xrn[0] = sum_money_in_inputs(tx);
    sum_xrn[1] = sum_money_in_outputs(tx);
