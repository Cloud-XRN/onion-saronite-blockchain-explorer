...
#ifndef XRNEG01_TOOLS_H
#define XRNEG01_TOOLS_H

#define PATH_SEPARARTOR '/'

#define XRN_AMOUNT(value) \
...
inline double
get_xrn(uint64_t core_amount)
{
    return XRN_AMOUNT(core_amount);
}
...
static
string
xrn_amount_to_str(const uint64_t& xrn_amount,
                  string _format="{:0.9f}",
                  bool zero_to_question_mark=true)
{
    ...
    {
        amount_str = fmt::format(_format, XRN_AMOUNT(xrn_amount));
    }
    else
    {
        if (xrn_amount > 0 && zero_to_question_mark == true)
        {
            amount_str = fmt::format(_format, XRN_AMOUNT(xrn_amount));
        }
    }
